...
Using `ggarrange()` of [**ggpubr**](https://cran.r-project.org/web/packages/ggpubr/) package, multiple histograms will be plotted. The code chunk below is used to create 12 histograms. Then, `ggarrange()` is used to organised these histogram into a 3 columns by 4 rows small multiple plot.
...
1.  **From first, model summary, we can interpret dependent variable is 65.1% in relation with independent variable.**
2.  **From Anova summary, we can check the significance is less than 0.05. Therefore, null hypothesis, all population means are equal, will be rejected.**
